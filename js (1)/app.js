...
var database_recommendations = firebase.database().ref("recommendations");
...
let   recommendations;
var numRecommendations_max = 10, metric_max = 7;
...
var locations_in_austin = {
    "central": {"lat": 30.284919, "lng": -97.734057},  
    "north"  : {"lat": 30.402065, "lng": -97.725883},  
    "west"   : {"lat": 30.343171, "lng": -97.835514},  
    "east"   : {"lat": 30.263466, "lng": -97.695904},  
    "south"  : {"lat": 30.256079, "lng": -97.763509}   
};
...
*****************************************************************************
*****************************************************************************/
var eventTypes = ["eat", "play", "drink", "location"];
let   eventNames = ["", "", "", ""];

$(".dropdown-item").click(function() {
    var eventName = $(this).text();
    var index     = Math.floor($(".dropdown-item").index(this) / 5);
...
function spherical_distance(point1, point2) {
    var lat1_rad = point1.lat * deg_to_rad;
    var lng1_rad = point1.lng * deg_to_rad;

    var lat2_rad = point2.lat * deg_to_rad;
    var lng2_rad = point2.lng * deg_to_rad;
...
function displayRecommendations(eventNames) {
    var directoryName = `${eventNames[0]}_${eventNames[1]}_${eventNames[2]}`;
    var myLocation    = locations_in_austin[eventNames[3]];
    ...
    database_recommendations.child(directoryName).once("value", function(snapshot) {
        ...
        var data = snapshot.val().filter((a) => spherical_distance(a.center, myLocation) < metric_max);
...
$("body").on("click", ".subContainer", function() {
    ...
    var r      = recommendations[$(".subContainer").index(this)];
    var places = {"eat": r.eat, "play": r.play, "drink": r.drink};
    ...
    for (let key in places) {
        ...
        google.maps.event.addListener(marker, "click", function() {
            var phone   = (p.phone)   ? `${p.phone}<br>` : "";
            var website = (p.website) ? `<a href="${p.website}" target="_blank">Visit their website</a>` : "";
            var output  = `<div><strong>${p.name}</strong><br>${p.location.street}<br>${phone}${website}</div>`;
...
$("#button_login").click(function() {
    ...
    var email    = $("#userEmail_login").val();
    var password = $("#userPassword_login").val();
...
$("#button_signup").click(function() {
    ...
    var email    = $("#userEmail_signup").val();
    var password = $("#userPassword_signup").val();
